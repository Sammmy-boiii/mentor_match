...
const VideoCall = ({ 
    ...
    const createPeerConnection = useCallback((targetSocketId) => {
        ...
        // Handle remote stream (for now show the first remote stream)
        pc.ontrack = (event) => {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            try {
                ...
                // Handle incoming offer (with glare/rollback handling)
                socket.on('offer', async ({ offer, senderSocketId }) => {
                    ...
                    try {
                        ...
                        const waitForSignalingState = async (targetState, timeout = 2000) => {
                            const interval = 50;
                            let waited = 0;
                            while (pc && pc.signalingState !== targetState && waited < timeout) {
                                await new Promise((res) => setTimeout(res, interval));
                                waited += interval;
                            }
                            return pc ? pc.signalingState === targetState : false;
                        };

                        // If we already have a peer connection with a local offer, this is a glare.
                        if (pc && pc.localDescription && pc.localDescription.type === 'offer') {
                            ...
                            try {
                                ...
                                // wait for rollback to complete and signalingState to become 'stable'
                                await waitForSignalingState('stable', 2000);
                            } catch (rbErr) {
                                console.warn('Rollback failed, will close and recreate peer connection', rbErr);
                        ...
                        if (!pc) {
                            // create new peer connection if none
                        ...
                        // Wait until we are in have-remote-offer (or remoteDescription is set)
                        const ready = await (async () => {
                            const interval = 50;
                            let waited = 0;
                            const timeout = 2000;
                            while (waited < timeout) {
                                if (pc.remoteDescription && pc.remoteDescription.type === 'offer') return true;
                                if (pc.signalingState === 'have-remote-offer') return true;
                        ...
                        if (!ready) {
                            console.warn('Timed out waiting for have-remote-offer state, proceeding cautiously');
                        }

                        // Create and set local answer
                        const answer = await pc.createAnswer();
                        // ensure we're in correct state before setting local description
                        if (pc.signalingState === 'have-remote-offer' || (pc.remoteDescription && pc.remoteDescription.type === 'offer')) {
                            await pc.setLocalDescription(answer);
                        } else {
                            console.warn('Skipping setLocalDescription: signalingState=', pc.signalingState);
                        }
                        ...
                        for (const qc of queued) {
                            try {
                                ...
                                console.warn('Error adding queued ICE candidate after answer:', addErr);
                            }
                        }
                ...
                socket.on('answer', async ({ answer, senderSocketId }) => {
                    ...
                    try {
                        ...
                        // Wait briefly for the PC to reach 'have-local-offer' state
                        const waitForLocalOffer = async (timeout = 1000) => {
                            const interval = 100;
                            let waited = 0;
                            while (pc.signalingState !== 'have-local-offer' && waited < timeout) {
                                await new Promise((res) => setTimeout(res, interval));
                                waited += interval;
                            }
                        };

                        await waitForLocalOffer(1000);

                        // Only apply answer if we actually have a local offer
                        if (pc.signalingState === 'have-local-offer' || (pc.localDescription && pc.localDescription.type === 'offer')) {
                            ...
                            for (const qc of queued) {
                                try {
                                    ...
                                    console.warn('Error adding queued ICE candidate after setting remote description:', addErr);
                                }
                            }
                            candidateQueueRef.current.set(senderSocketId, []);
                        } else {
                            console.warn('Ignoring answer: localDescription is not an offer. signalingState=', pc.signalingState);
                        }
                    } catch (err) {
