...
const AdminContextProvider = (props) => {
  ...
  // Update Tutor
  const updateTutor = async (tutorId, tutorData) => {
    try {
      let payload;
      if (tutorData instanceof FormData) {
        tutorData.append('tutorId', tutorId);
        payload = tutorData;
      } else {
        payload = { tutorId, ...tutorData };
      }

      const { data } = await axios.post(
        backendUrl + "/api/admin/update-tutor",
        payload,
        { headers: { aToken } }
      );
      if (data.success) {
  ...
  const deleteTutor = async (tutorId) => {
    try {
      const { data } = await axios.post(
        backendUrl + "/api/admin/delete-tutor",
        { tutorId },
        { headers: { aToken } }
      );
      if (data.success) {
