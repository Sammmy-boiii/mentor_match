    ...
    useEffect(() => {
        const initRoom = async () => {
            ...
            try {
                ...
                // Generate room ID for tutor
                const { data: roomResponse } = await axios.post(
                    `${backendUrl}/api/video-call/tutor/generate-room`,
                    { sessionId },
                    { headers: { tToken } }
                );
                ...
                const { data: joinResponse } = await axios.post(
                    `${backendUrl}/api/video-call/tutor/join`,
        ...
        initRoom();
    }, [sessionId, tToken, backendUrl, navigate]);

    // Handle call end
    const handleCallEnd = async (duration) => {
        toast.success(`Session completed. Duration: ${Math.floor(duration / 60)} minutes ${duration % 60} seconds`);
