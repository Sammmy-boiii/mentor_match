...
const Dashboard = () => {
  // --- Token-from-URL logic ---
  useEffect(() => {
    const url = new URL(window.location.href);
    const aToken = url.searchParams.get('aToken');
    if (aToken) {
      localStorage.setItem('aToken', aToken);
      // Remove token from URL and reload so context picks it up
      url.searchParams.delete('aToken');
  ...
  return (
    dashData && (
      <div className="m-5 w-auto">        {/* Date & Time Display */}
        {/* Stats Cards */}
